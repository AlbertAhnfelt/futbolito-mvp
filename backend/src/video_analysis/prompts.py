...
You will receive a 30-second video clip of a football match.
...
"time" → Time MUST be relative to THIS CLIP's start time (00:00:00 to 00:00:30).
"replay" → true for replay segments, false for live action.
"intensity" → integer from 1 (low) to 10 (very intense).

Formatting:
Return only valid JSON with no comments, no extra text, and no trailing commas.
"""
